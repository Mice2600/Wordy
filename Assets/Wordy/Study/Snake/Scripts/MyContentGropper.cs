...
public class MyContentGropper : MonoBehaviour
{
    ...
    private Transform ConcleateLast(float NextSize)
    {
        ...
        for (int i = 0; i < Lines.Count; i++)
        {
            ...
            if (NSize + NextSize + Lines[i].GetComponent<HorizontalLayoutGroup>().spacing > (ScreenSize)) continue;
            Founded = Lines[i];
            break;
        }
        if (Founded == null)
        ...
        GameObject AddNewList()
        {
            ...
            NLists.GetComponent<RectTransform>().offsetMax = Vector2.one * .5f;
            NLists.GetComponent<RectTransform>().offsetMin = Vector2.zero;
