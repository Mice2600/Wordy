...
//https://github.com/jpgordon00/UnityAsyncPrefabLoading
...
{
    public string FullPath; // the resource full path
...
public class GroupLoader
{
    ...
    */
    public async void LoadResources(List<Resource> resources, Action finishCallback = null)
    {
        ...
        _numResources = (float)resources.Count;
        StopWatch.Start();
        foreach (var resource in resources) LoadResource(resource.FullPath, resource.FinishCallback, resource.CacheResult, resource.InstantiateResult, resource.Transform);
        while (!IsAllDone())
        {
            await Task.Delay(75);
        }
